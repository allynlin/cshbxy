...
const Procurement: React.FC = () => {
    ...
    const showResolveConfirm = (e: string) => {
        Dialog.confirm({
            content: '同意当前申请吗？',
            onConfirm: async () => {
                await resolveProcurement(e).then((res: any) => {
                    if (res.code === 200) {
                        Toast.show({
                            icon: 'success',
                            content: res.msg,
    ...
    const RenderDialogContent = (item: any) => {
        return (
            <div style={style}>
                ...
                <p>物品：</p>
                <div className={classes.outPutHtml} dangerouslySetInnerHTML={{__html: item.items}}/>
                <p>价格：{item.price}</p>
    ...
    return (
        <PullToRefresh
            ...
            onRefresh={async () => {
                await findProcurementWaitApprovalList().then((res: any) => {
                    if (res.code === 200) {
                        const arr = res.body.map((item: any, index: number) => {
                            return {
                                ...item,
